...
namespace Maxicycles.Pages.Checkout
{
    public class ConfirmationModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet(int orderId)
        {
            ...
            if (order.MaxicyclesUser.Id != user.Id)
            {
                return Unauthorized();
            }
            ...
            {
                return NotFound("Sorry a receipt has already been sent");
            }
        ...
        public string GenerateEmailReceipt(Order order)
        {
            ...
            foreach (var item in order.OrderItems)
            {
                ...
                if (item is OrderService service)
                {
                    sb.AppendLine($"<td>{service.ServiceDate.ToLocalTime()}</td>");
                }
                sb.AppendLine("</tr>");
            }
